...
- It embeds the TL;DR, beautifully formats your typed concepts, and automatically weaves the **semantic tags and Gemini Nano Banana generated images** directly into the Markdown layout to heavily enrich your notes.
...
Visual context is crucial for memory. The processing unit preserves and carries over **existing page images** that semantically align with the extracted nuggets. If a nugget lacks a visual aid, the extension falls back to a background process using the local **Gemini Nano Banana** model to generate a highly representative, stunning image specifically tailored to that nugget's context.
